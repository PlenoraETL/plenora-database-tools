...
fn expect_single_row(rows: Vec<Vec<ParameterValue>>) -> Result<Vec<ParameterValue>> {
    if rows.is_empty() {
...
fn expect_at_most_one_row(rows: Vec<Vec<ParameterValue>>) -> Result<Option<Vec<ParameterValue>>> {
    if rows.is_empty() {
        return Ok(None);
    }
    if rows.len() > 1 {
...
fn expect_single_column(row: Vec<ParameterValue>) -> Result<ParameterValue> {
    if row.len() != 1 {
    ...
    Ok(row.into_iter().next().expect("controllato lunghezza"))
}
...
pub async fn query_optional(
    ...
    statement: &Statement,
    cancellation: &CancellationToken,
) -> Result<Option<Vec<ParameterValue>>> {
    let rows = tx.query(statement, cancellation).await?;
...
mod tests {
    ...
    fn expect_single_row_rejects_multiple() {
        let err = expect_single_row(vec![
            vec![ParameterValue::I32(1)],
            vec![ParameterValue::I32(2)],
        ])
        .unwrap_err();
    ...
    fn expect_single_column_rejects_wrong_width() {
        let err =
            expect_single_column(vec![ParameterValue::I32(1), ParameterValue::I32(2)]).unwrap_err();
