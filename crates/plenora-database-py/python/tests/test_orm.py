...
class _FakeTransaction:
    ...
    def execute_scalar(self, statement, params=None):
        parameters = (
            ...
            else dict(enumerate(params))
        )
        self.executed.append((statement, parameters))
        return self.scalar_values.pop(0) if self.scalar_values else self.scalar
...
def test_oracle_23_ddl_is_explicit_and_unqualified_features_fail_closed() -> None:
    ...
    generated_ddl = p.OrmMetadata(models=(OracleGenerated,)).ddl("oracle")[0]
    assert '"id" NUMBER(10) GENERATED BY DEFAULT AS IDENTITY' in generated_ddl
    ...
    aware_ddl = p.OrmMetadata(models=(OracleAwareTimestamp,)).ddl("oracle")[0]
    assert '"observed_at" TIMESTAMP WITH TIME ZONE' in aware_ddl
...
def test_oracle_generated_key_reads_the_identity_sequence_currval() -> None:
    transaction = _FakeTransaction([{"id": 75, "created_by": "database"}])
    transaction.scalar_values = ["ISEQ$$_12345", 75]
    orm = p.OrmSession(_FakeSession(transaction, provider="oracle"))
    account = GeneratedAccount(name="Ada")
    orm.add(account)
    ...
    assert "USER_TAB_IDENTITY_COLS" in transaction.executed[1][0]
    assert transaction.executed[1][1] == {0: "orm_generated_accounts", 1: "id"}
    assert transaction.executed[2][0] == 'SELECT "ISEQ$$_12345".CURRVAL FROM DUAL'
