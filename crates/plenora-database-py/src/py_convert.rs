...
fn typed_to_param(kind: &str, value: &Bound<'_, PyAny>) -> PyResult<ParameterValue> {
    match kind {
        ...
        _ => Err(PyTypeError::new_err(
            "TypedValue kind sconosciuto (attesi: uuid, int64, date, timestamp, timestamp_tz, decimal, null)",
        )),
    }
}
